...
def menu():
    ...
    print(Fore.RED+"-"*30+
          ...
          +"-"*30+
          "\n-------"+Fore.BLUE+" 6*Macerayı bitir. "+Fore.RED+"----\n"
          +"-"*30)
while True:
    ...
    menu()
    sec=input("Hangisini seçeceksiniz? {1-2-3-4-5-6}")
    ...
    if sec=="1":
        ...
        if hile_sayar==2:
            p_can=0
            print("Hırsın gözünü kör etti. Kendi kılıcının soğuk çeliği zindanın\n"
                  "rutubetli zeminine düşerken, adını fısıldayacak kimse kalmadı.\n"
                  "Hikâyen buraya kadardı...")
            kayip_ekle()
            break
        ...
        hile_sayar=0
        if p_can<=0:
            print(Fore.RED+"Son darbe göğsüne indiğinde karanlık etrafını sardı."
                  " Bedenin bu zindanın karanlık"
                  " köşelerinde çürümeye terk edildi."
                  " Gelecek maceracılar kemiklerinin"
                  " üzerinden basıp geçecek...")
            kayip_ekle()
            break
        ...
        print("Kral 1 2 3 4 5 6 ya basıcaksın o kadar!")
